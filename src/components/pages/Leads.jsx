...
export default function Leads() {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const dataToSave = {
        ...
        medico_agendamento: formData.medico_agendamento || '',
        valor_agendamento: parseFloat(formData.valor_agendamento) || 0,
        local_agendado: formData.local_agendado || '',
  ...
  const handleAgendamentoMigration = useCallback(async () => {
    try {
      setMigrating(true)
      setError(null)
      
      const result = await firebaseDataService.migrateLeadsForAgendamento()
      ...
      setError(`Erro durante a migração de agendamento: ${err.message}`)
    } finally {
      setMigrating(false)
    }
  }, [loadData])
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Card>
                      <CardHeader className="pb-4">
                        ...
                        <CardDescription>
                          Dados específicos para o agendamento do paciente
                        </CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => handleFormChange('data_agendada', e.target.value)}
                              className="h-10"
                            />
                          </div>
                          <div className="space-y-2">
                            <label className="text-sm font-medium">Procedimento</label>
                            <Select value={formData.procedimento_agendamento} onValueChange={(value) => handleFormChange('procedimento_agendamento', value)}>
                              <SelectTrigger className="h-10">
                                <SelectValue placeholder="Selecione o procedimento" />
                              </SelectTrigger>
                              <SelectContent className="max-h-[200px] overflow-y-auto">
                          ...
                          <div className="space-y-2">
                            <label className="text-sm font-medium">Médico</label>
                            <Select value={formData.medico_agendamento} onValueChange={(value) => handleFormChange('medico_agendamento', value)}>
                              <SelectTrigger className="h-10">
                                <SelectValue placeholder="Selecione o médico" />
                              </SelectTrigger>
                              <SelectContent className="max-h-[200px] overflow-y-auto">
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              type="number"
                              step="0.01"
                              value={formData.valor_agendamento}
                              onChange={(e) => handleFormChange('valor_agendamento', e.target.value)}
                              placeholder="0,00"
                              className="h-10"
                            />
                          </div>
                          <div className="space-y-2 md:col-span-2">
                            ...
                            <Input
                              value={formData.local_agendado}
                              onChange={(e) => handleFormChange('local_agendado', e.target.value)}
                              placeholder="Ex: Consultório 1, Sala de Cirurgia, etc."
                              className="h-10"
                            />
                          </div>
                        </div>
